...
def get_inputs(year):
    device = 'cuda:0'
    node2predict_c = {}
    category_list = ['高端装备制造', '节能环保', '生物', '数字创意', '新材料', '新能源', '新能源汽车', '新一代信息技术']
    ...
    for category in tqdm(range(8), desc=str(year)):
        if category in [0, 1, 2, 4, 5]:
            node2vec_path = 'data/node2vec_{}_v2.json'.format(year)
        else:
        ...
        model = MyDnnSimple()
        if category in [0, 1, 2, 4, 5]:
            model_state_dict = torch.load('model_final_v2/Dnn_{}_v1.pt'.format(category))
            c = np.load('data/category2vec_v1.npy')
        else:
            model_state_dict = torch.load('model_final_v2/Dnn_{}_v0.pt'.format(category))
            c = np.load('data/category2vec_v0.npy')
        ...
        if category != 7:
            ...
            # 非常抽象的做法，先这样。
            threshold_list_7 = [0.999 - i * 0.0001 for i in range(12)]
            y_pred = torch.where(y_pred > threshold_list_7[year - 2011],
                                 torch.ones_like(y_pred), torch.zeros_like(y_pred))
        # print(y_pred)
    ...
    # save by json
    with open('data/result/node2predict_c_{}.json'.format(year), 'w', encoding='utf-8') as f:
...
def analysis():
    ...
    for year in range(2011, 2023):
        with open('data/result/node2predict_c_{}.json'.format(year), 'r', encoding='utf-8') as f:
...
def output():
    ...
    df = pd.read_csv('data/全部AB股.csv', dtype=str)
    ...
    for year in range(2011, 2023):
        ...
        df.to_excel('data/output/{}_result.xlsx'.format(year), index=False)


def test():
    # load node2predict_c
    node2predict_c = {}
    with open('data/result/node2predict_c_2022.json', 'r', encoding='utf-8') as f:
        node2predict_c[2022] = json.load(f)
    ...
    result_list = []
    for category in range(8):
        for node, pred in node2predict_c[2022][str(category)].items():
            if pred == 1:
                try:
